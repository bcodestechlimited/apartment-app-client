...
import { Button } from "@/components/ui/button";
import { FileInput, type FileValue } from "@/components/custom/file-input"; // Ensure FileValue is exported from FileInput
import {
  amenities,
  facilities,
  pricingModels,
...
// Define a local interface that matches your UI state (URLs instead of raw Files)
type AddPropertyFormValues = Omit<IAddProperty, "pictures"> & {
  pictures: FileValue[];
};
...
export default function AddPropertyPage() {
  ...
  const form = useForm<AddPropertyFormValues>({
    defaultValues: {
      ...
      numberOfBathrooms: "",
      seatingCapacity: "",
      pictures: [], // Now holds { url: string, name: string }
      otherFees: [],
      isEnsuite: false,
    },
  });
  ...
  const onSubmit = async (data: AddPropertyFormValues) => {
    ...
    const payload = {
      ...
      type: data.type.replace(" ", "-").toLowerCase(),
      isEnsuite: String(data.isEnsuite),
      // Transform the FileValue objects into an array of URL strings for the backend
      ...
      seatingCapacity:
        data.type.toLowerCase().replace(" ", "-") === "co-working-space"
          ? String(data.seatingCapacity)
          : "1",
    };

    console.log("payload", payload);

    // Note: propertyService.addProperty must be updated to accept a JSON body
    // instead of FormData on the backend.
    propertyMutation.mutateAsync(payload);
  };
    ...
    <div className="container px-4 md:px-8">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          {/* Section 5: Photos - Integrated with Cloudinary */}
          <Card>
            <CardHeader>
              <CardTitle>Property Photos</CardTitle>
            </CardHeader>
            <CardContent>
              <FormField
                ...
                rules={{
                  ...
                  validate: (val) =>
                    val.length >= 3 || "Please upload at least 3 images",
                }}
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <FileInput
                        accept=".jpg,.jpeg,.png"
                        value={field.value} // Now using FileValue[]
                        multiple
                        customMessage="min of 3 images (exterior, interior, features)"
                        numberOfFiles={10}
                        onFilesChange={(updatedFiles) =>
