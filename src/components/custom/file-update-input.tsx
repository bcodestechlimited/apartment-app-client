...
import { useRef, useState } from "react";
import { XIcon, ImageIcon, FileTextIcon, Loader2 } from "lucide-react";
import { Input } from "../ui/input";
import { cn } from "@/lib/utils";
...
interface FileUpdateInputProps {
  existingImages?: string[];
  onChange: (allImages: string[]) => void; // Simplified: returns the final array of URLs
  multiple?: boolean;
  maxSize?: number;
  accept?: string;
  className?: string;
...
export const FileUpdateInput = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updated = [...currentImages, ...filteredResults];
      setCurrentImages(updated);
      onChange(updated);
    } catch (error) {
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div
        ...
        className={cn(
          "cursor-pointer flex flex-col items-center justify-center border-2 border-dashed border-gray-300 p-6 rounded-lg hover:border-primary transition-colors",
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
        {currentImages.map((img, idx) => (
          <div
            ...
            className={cn(
              "relative group aspect-square rounded-lg overflow-hidden border bg-gray-100 cursor-move",
              draggingIndex === idx && "opacity-50 ring-2 ring-primary",
            )}
          >
            <img
            ...
            <button
              ...
              className="absolute top-1 right-1 bg-white/80 backdrop-blur-sm rounded-full p-1 shadow-sm hover:bg-red-50 hover:text-red-600 transition-colors"
            >
              <XIcon size={14} />
            </button>
          </div>
